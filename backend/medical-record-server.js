...
async function createMedicalRecordTables(query) {
  try {
    ...
    await query(`CREATE INDEX IF NOT EXISTS idx_expedientes_paciente ON expedientes_medicos(paciente_id, sucursal_id)`);
...
function setupMedicalRecordRoutes(app, query) {
  ...
  app.get('/api/expediente-medico/paciente/:nombrePaciente', ah(async (req, res) => {
    ...
    try {
      ...
      const expedienteResult = await query(`
        ...
        LIMIT 1
      `, [decodeURIComponent(nombrePaciente), sucursalId]);
  ...
  app.post('/api/expediente-medico', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
    const {
      paciente_id,
      nombre_paciente,
    ...
    try {
      ...
      const result = await query(`
        INSERT INTO expedientes_medicos (
          ...
          telefono_emergencia, sucursal_id
...
app.put('/api/expediente-medico/:id', ah(async (req, res) => {
  const { id } = req.params;
  const sucursalId = getSucursal(req);
...
app.post('/api/historia-clinica-dental', ah(async (req, res) => {
  const sucursalId = getSucursal(req);
...
app.put('/api/historia-clinica-dental/:id', ah(async (req, res) => {
  const { id } = req.params;
  const sucursalId = getSucursal(req);
  ...
  app.post('/api/odontograma', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
    const {
      expediente_id,
      diente_numero,
    ...
    try {
      ...
      const result = await query(`
        INSERT INTO odontograma (
          expediente_id, diente_numero, estado, superficie, observaciones,
  ...
  app.put('/api/odontograma/:id', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
    ...
    try {
      ...
      const result = await query(`
        ...
        RETURNING *
      `, [estado, superficie, observaciones, fecha_registro, doctor_id, id, sucursalId]);
  ...
  app.post('/api/tratamiento-dental', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
    const {
      expediente_id,
      fecha,
    ...
    try {
      ...
      const result = await query(`
        INSERT INTO tratamientos_dentales (
          expediente_id, fecha, diente_numero, procedimiento, descripcion,
  ...
  app.put('/api/tratamiento-dental/:id', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
  ...
  app.delete('/api/tratamiento-dental/:id', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
  ...
  app.post('/api/consentimiento-informado', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
    const {
      expediente_id,
      tipo_tratamiento,
    ...
    try {
      ...
      const result = await query(`
        INSERT INTO consentimientos_informados (
          expediente_id, tipo_tratamiento, descripcion_tratamiento, riesgos_beneficios,
  ...
  app.put('/api/consentimiento-informado/:id/firmas', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
    ...
    try {
      ...
      const result = await query(`
        ...
        WHERE id = $3 AND sucursal_id = $4
        RETURNING *
      `, [firma_paciente, firma_doctor, id, sucursalId]);
  ...
  app.post('/api/documento-radiografia', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
    const {
      expediente_id,
      tipo,
    ...
    try {
      ...
      const result = await query(`
        INSERT INTO documentos_radiografias (
          expediente_id, tipo, nombre, descripcion, fecha_toma,
          datos_base64, url, doctor_id, sucursal_id
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        RETURNING id, expediente_id, tipo, nombre, descripcion, fecha_toma, doctor_id, sucursal_id, created_at
      `, [
        expediente_id,
        tipo,
        nombre,
  ...
  app.get('/api/documento-radiografia/:id/download', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
  ...
  app.delete('/api/documento-radiografia/:id', ah(async (req, res) => {
    const { id } = req.params;
    const sucursalId = getSucursal(req);
  ...
  // Buscar pacientes
  app.get('/api/expediente-medico/buscar-pacientes', ah(async (req, res) => {
    ...
    try {
      const result = await query(`
        ...
        WHERE sucursal_id = $1 
        AND (LOWER(nombre_paciente) LIKE LOWER($2) OR telefono LIKE $2)
        ORDER BY nombre_paciente
        LIMIT 20
      `, [sucursalId, `%${searchQuery}%`]);
  ...
  app.get('/api/expediente-medico/estadisticas', ah(async (req, res) => {
    const sucursalId = getSucursal(req);
