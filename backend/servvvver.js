...
// ================================
// 🤖 CARGA DEL MÓDULO IA (LEGACY, opcional)
// ================================

// Flag de legacy AI (evita ReferenceError si no existe env)
const AI_ENABLE_LEGACY = String(process.env.AI_ENABLE_LEGACY || '').toLowerCase() === 'true';

let aiModule = null;

if (AI_ENABLE_LEGACY) {
  console.log('🤖 AI_ENABLE_LEGACY=true -> intentando cargar ./ai-conversations-module.js ...');
  try {
    aiModule = require('./ai-conversations-module.js');
    console.log('✅ Módulo IA LEGACY cargado. Funciones:', Object.keys(aiModule || {}));
  } catch (error) {
    console.error('❌ ERROR cargando IA LEGACY:', error.message);
    aiModule = null;
  }
} else {
  console.log('🤖 AI_ENABLE_LEGACY=false -> IA LEGACY deshabilitada (solo SaaS).');
}

// 🆕 SaaS AI Routes (multi-tenant)
let aiSaas = null;
(() => {
  const candidates = [
    './modules/ai-saas-routes.js',
    './modules/ai-saas-routes',
    './sass_modules/ai-saas-routes.js',
    './sass_modules/ai-saas-routes',
    './modulesai-saas-routes.js',
    './modulesai-saas-routes',
  ];
  for (const p of candidates) {
    try {
      // eslint-disable-next-line import/no-dynamic-require, global-require
      aiSaas = require(p);
      console.log('✅ Módulo AI SaaS cargado:', p);
      break;
    } catch (e) { /* ignore */ }
  }
  if (!aiSaas) console.error('❌ No se pudo cargar AI SaaS (revisa que exista /modules/ai-saas-routes.js).');
})();
...
function pickDbKey(req) {
  ...
  const m1 = String(process.env.DB1_ORIGINS_MATCH || 'dentalux-sucsursales,app1').toLowerCase().split(',').map(s => s.trim()).filter(Boolean);
  const m2 = String(process.env.DB2_ORIGINS_MATCH || 'staticdemo-94eq,app2').toLowerCase().split(',').map(s => s.trim()).filter(Boolean);
  const m3 = String(process.env.DB3_ORIGINS_MATCH || 'cliniqoneenterprice,app3').toLowerCase().split(',').map(s => s.trim()).filter(Boolean);
if (hay) {
...
// ================================
// 🆕 IA SaaS (preferente): /api/ai/*
// ================================
let AI_SAAS_OK = false;
if (aiSaas && typeof aiSaas.setupAiSaasRoutes === 'function') {
  try {
    aiSaas.setupAiSaasRoutes(app, q);
    AI_SAAS_OK = true;
    console.log('✅ Rutas AI SaaS configuradas: /api/ai/chat y /api/ai/conversations');
  } catch (e) {
    console.error('❌ Error configurando rutas AI SaaS:', e.message);
    AI_SAAS_OK = false;
  }
}

// ================================
// 🧩 IA LEGACY (solo si AI_ENABLE_LEGACY=true)
// ================================
let AI_LEGACY_OK = false;
if (AI_ENABLE_LEGACY && aiModule && typeof aiModule.setupAiRoutes === 'function') {
  try {
    if (typeof aiModule.createAiTables === 'function') {
      // DB1
      aiModule.createAiTables(q)
        .then(() => console.log('✅ Tablas IA LEGACY OK (DB1)'))
        .catch((e) => console.error('❌ Error creando tablas IA LEGACY (DB1):', e));

      // DB2
      if (poolDB2) {
        als.run({ pool: poolDB2, dbKey: 'db2' }, () => aiModule.createAiTables(q))
          .then(() => console.log('✅ Tablas IA LEGACY OK (DB2)'))
          .catch((e) => console.error('❌ Error creando tablas IA LEGACY (DB2):', e));
      }

      // DB3
      if (poolDB3) {
        als.run({ pool: poolDB3, dbKey: 'db3' }, () => aiModule.createAiTables(q))
          .then(() => console.log('✅ Tablas IA LEGACY OK (DB3)'))
          .catch((e) => console.error('❌ Error creando tablas IA LEGACY (DB3):', e));
      }
    }

    aiModule.setupAiRoutes(app, q);
    AI_LEGACY_OK = true;
    console.log('✅ Rutas IA LEGACY configuradas (AI_ENABLE_LEGACY=true).');
  } catch (e) {
    AI_LEGACY_OK = false;
    console.error('❌ Error configurando IA LEGACY:', e.message);
  }
}

// Fallback 503 SOLO si no hay NINGÚN módulo de IA montado
if (!AI_SAAS_OK && !AI_LEGACY_OK) {
  console.error('❌ IA no disponible: ni SaaS ni Legacy montaron rutas.');
...
// Opcional:
//   PUBLIC_BASE_URL=https://tu-servicio.onrender.com (si no existe, usa req.get('host'))
// ==============================
...
async function getOrCreateFbConversationId(psid, pageId, req) {
  ...
  const headers = { 'Content-Type': 'application/json' };
  if (dbKey) headers['x-db'] = String(dbKey);
  headers['x-channel'] = 'messenger';
  if (pageId) headers['x-page-id'] = String(pageId);
  // En Messenger NO forzamos sucursal; el bot la pregunta.
...
async function getOrCreateAiConversationIdForMessenger(pageId, psid, dbKey) {
  ...
  const legacySenderId = `${pid}:${sid}`; // para compatibilidad con tablas viejas

  // 1) Asegurar tabla + columnas (sin romper instalaciones viejas)
  await pool.query(`
  ...
  // Si existía una versión vieja (ej: sender_id + conversation_id), agregamos columnas faltantes
  await pool.query(`ALTER TABLE messenger_threads ADD COLUMN IF NOT EXISTS clinic_id TEXT`);
  await pool.query(`ALTER TABLE messenger_threads ADD COLUMN IF NOT EXISTS page_id TEXT`);
  await pool.query(`ALTER TABLE messenger_threads ADD COLUMN IF NOT EXISTS psid TEXT`);
  await pool.query(`ALTER TABLE messenger_threads ADD COLUMN IF NOT EXISTS conversation_id BIGINT`);
  await pool.query(`ALTER TABLE messenger_threads ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`);

  // Índice único moderno (page_id, psid) — en instalaciones viejas no hay esos campos
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_messenger_threads_page_psid ON messenger_threads(page_id, psid) WHERE page_id IS NOT NULL AND psid IS NOT NULL`);

  // 2) Buscar mapping existente (nuevo o legacy)
  const found = await pool.query(
    `SELECT sender_id, clinic_id, conversation_id
       FROM messenger_threads
      WHERE (page_id=$1 AND psid=$2)
         OR sender_id=$3
      LIMIT 1`,
    [pid, sid, legacySenderId]
  );

  // 3) Resolver tenant real desde clinic_channels
  const { rows: rClinic } = await pool.query(
    `SELECT clinic_id
       FROM clinic_channels
      WHERE channel='messenger' AND external_id=$1
      LIMIT 1`,
    [pid]
  );
  const clinicId = rClinic?.[0]?.clinic_id || null;

  // 4) Si existe y coincide, reusamos conversación
  if (found.rows?.length) {
    const row = found.rows[0];
    if (row.clinic_id && clinicId && String(row.clinic_id) !== String(clinicId)) {
      // mismatch -> creamos otra conversación y actualizamos mapping a clinic actual
      // (esto evita el 403 Tenant mismatch)
      console.warn('⚠️ [messenger_threads] clinic_id mismatch -> recreando conversación:', { rowClinic: row.clinic_id, clinicId });
    } else if (row.conversation_id) {
      // Aseguramos que esté actualizado a formato moderno
      await pool.query(
        `UPDATE messenger_threads
            SET clinic_id = COALESCE($3, clinic_id),
                page_id = COALESCE($1, page_id),
                psid = COALESCE($2, psid),
                updated_at = NOW()
          WHERE sender_id = $4`,
        [pid, sid, clinicId, row.sender_id]
      );
      return Number(row.conversation_id);
    }
  }

  // 5) Crear conversación nueva via endpoint interno (usa /api/ai/conversations de SaaS)
  // Para no depender de HTTP interno aquí, insertamos directamente en ai_conversations.
  const { rows: convRows } = await pool.query(
    `INSERT INTO ai_conversations(title, clinic_id, channel, external_id, state)
     VALUES ($1, $2, 'messenger', $3, '{}'::jsonb)
     RETURNING id`,
    ['Messenger', clinicId, pid]
  );
  const conversationId = Number(convRows?.[0]?.id);

  // 6) Upsert mapping (guardamos ambos formatos)
  // Preferimos clave moderna (page_id, psid) pero mantenemos sender_id legacy.
  await pool.query(
    `INSERT INTO messenger_threads(sender_id, clinic_id, page_id, psid, conversation_id, updated_at)
     ...
     ON CONFLICT (sender_id) DO UPDATE SET
       ...
       updated_at=NOW()`,
    [legacySenderId, clinicId, pid, sid, conversationId]
  );

  // Intento adicional: si existe row con page_id+psid pero sender_id distinto, lo actualizamos
  await pool.query(
    `INSERT INTO messenger_threads(sender_id, clinic_id, page_id, psid, conversation_id, updated_at)
     VALUES ($1, $2, $3, $4, $5, NOW())
     ON CONFLICT ON CONSTRAINT uq_messenger_threads_page_psid DO UPDATE SET
       clinic_id=EXCLUDED.clinic_id,
       conversation_id=EXCLUDED.conversation_id,
       updated_at=NOW()`,
    [legacySenderId, clinicId, pid, sid, conversationId]
...
async function callClinicAiForMessenger(senderId, pageId, msgText, req) {
  ...
  const headers = { 'Content-Type': 'application/json', 'x-db': String(dbKey), 'x-channel': 'messenger', 'x-page-id': String(pageId||''), 'x-from': String(senderId||'') };
