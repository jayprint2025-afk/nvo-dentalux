...
async function ensureAiConversationForPhone({ phone, sucursalId, phoneNumberId }) {
  ...
  // Buscar por phone_number_id si está disponible, sino buscar por teléfono solamente
  const found = await q(
  ...
  if (found.rows[0]?.id) {
    ...
    const oldPhoneNumberId = found.rows[0].phone_number_id;
    
    // Si encontramos la conversación pero con un phone_number_id diferente, actualizarla
    if (phoneNumberId && oldPhoneNumberId && oldPhoneNumberId !== phoneNumberId) {
      console.log('🔄 [ensureAiConversation] Actualizando phone_number_id de conversación:', {
        conversation_id: convId,
        old_phone_number_id: oldPhoneNumberId,
        new_phone_number_id: phoneNumberId,
        phone: p
      });
      await q(
        `UPDATE ai_conversations SET phone_number_id = $1, updated_at = NOW() WHERE id = $2`,
        [phoneNumberId, convId]
      );
    }
  ...
  // Si no existe, buscar sin filtrar por phone_number_id (más permisivo)
  const foundAny = await q(
    `SELECT id, phone_number_id
     FROM ai_conversations
     WHERE state->>'wa_phone' = $1
     ORDER BY updated_at DESC
  ...
  if (foundAny.rows[0]?.id) {
    const convId = Number(foundAny.rows[0].id);
    const oldPhoneNumberId = foundAny.rows[0].phone_number_id;
    
    console.log('🔄 [ensureAiConversation] Encontrada conversación antigua, actualizando phone_number_id:', {
      conversation_id: convId,
      old_phone_number_id: oldPhoneNumberId,
      new_phone_number_id: phoneNumberId,
      phone: p
  ...
  // Si definitivamente no existe, crear nueva conversación
  ...
  console.log('✨ [ensureAiConversation] Nueva conversación creada:', {
