...
async function rescheduleAppointment(q, ctx, args = {}) {
  ...
  const serviceId = String(args.service_id || current.service_id || '').trim();

  if (!branchKey || !date || !startTime) {
    throw new Error('Faltan sucursal, fecha u hora para reagendar');
  }

  // La duración del destino debe salir del servicio configurado para la sucursal.
  // Si por compatibilidad no existe service_id, conservamos la duración actual.
  let durationHours = Number(current.duration_hours || 1);
  if (serviceId) {
    const serviceResult = await q(
      `SELECT id, COALESCE(duration_hours,1) AS duration_hours
         FROM services
    ...
    if (!serviceResult.rows?.[0]) {
      const error = new Error('El servicio seleccionado no está disponible en esa sucursal');
      error.code = 'SERVICE_NOT_IN_BRANCH';
      throw error;
    }
  ...
  await q('BEGIN');
  try {
    // Mismo candado lógico que la creación: serializa movimientos dentro
    // de empresa+sucursal+fecha y evita carreras entre crear/reagendar.
    const advisoryKey = `${tenantId}|${branchKey}|${date}`;
    await q(`SELECT pg_advisory_xact_lock(hashtext($1))`, [advisoryKey]);

    // Revalidación FINAL a nivel sucursal, por intervalo completo y excluyendo
    // únicamente la propia cita que estamos moviendo.
    const { rows: conflicts } = await q(
      `SELECT id, doctor_id, start_time::text AS start_time,
        ...
        WHERE tenant_id=$1::uuid
          ...
          AND date=$3::date
          AND id<>$4
          AND UPPER(COALESCE(status,'')) NOT IN ('CANCELADA','CANCELADO','CANCELED','CANCELLED')
          AND (
            (EXTRACT(HOUR FROM start_time)::int * 60 + EXTRACT(MINUTE FROM start_time)::int) < $5
            AND
            (EXTRACT(HOUR FROM start_time)::int * 60 + EXTRACT(MINUTE FROM start_time)::int
              + GREATEST(30, ROUND(COALESCE(duration_hours,1) * 60)::int)) > $6
          )
        FOR UPDATE`,
      [tenantId, branchKey, date, current.id, requestedEnd, requestedStart]
    );
    ...
    if (!rows?.[0]) {
      const error = new Error('No encontré la cita para reagendar');
      error.code = 'APPOINTMENT_NOT_FOUND';
      throw error;
    }
