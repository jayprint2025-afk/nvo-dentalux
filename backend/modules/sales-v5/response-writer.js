...
function fallback(profile, turn, objective) {
  if (turn.intent === 'pricing') {
    return 'CliniqOne tiene tres planes: Básico $890, Medio $1,090 y Completo $1,290 MXN al mes. También tenemos promoción de 2 sucursales por $1,490 MXN/mes. ¿Cuántas sucursales manejas?';
  }
...
async function writeReply({ profile, turn, objective, history = [] }) {
  if (!API_KEY) return fallback(profile, turn, objective);
  ...
  return String(json?.choices?.[0]?.message?.content || '').trim() || fallback(profile, turn, objective);
}
