...
// ================================
// 🤖 IA MODULE (LEGACY - OPTIONAL)
// ================================
console.log('🤖 Verificando módulo IA legacy (opcional)...');
let aiModule;
try {
  console.log('🔍 Intentando cargar ./ai-conversations-module.js...');
  aiModule = require('./ai-conversations-module.js');
  console.log('✅ Módulo IA legacy cargado exitosamente');
  console.log('🔍 Funciones disponibles IA:', Object.keys(aiModule));
} catch (error) {
  console.log('ℹ️  Módulo IA legacy no encontrado (esto es normal)');
  console.log('ℹ️  Usando sistema nuevo: ai-saas-routes + orchestrator');
  
  // Intentar con path absoluto por si acaso
  try {
    console.log('🔄 Intentando con path absoluto...');
    aiModule = require(__dirname + '/ai-conversations-module.js');
    console.log('✅ Módulo IA legacy cargado con path absoluto');
    console.log('🔍 Funciones disponibles IA:', Object.keys(aiModule));
  } catch (error2) {
    console.log('ℹ️  Confirmado: Usando solo sistema nuevo (ai-saas-routes + orchestrator)');
    aiModule = null;
  }
}
...
// ✅ Anticipo/depósito (solo si existen las tablas; NO rompe otras DB)
await q(`
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema='public' AND table_name='clinic_branches'
  ) THEN
    ALTER TABLE clinic_branches
      ADD COLUMN IF NOT EXISTS require_deposit_confirm boolean NOT NULL DEFAULT false,
...
const corsOptions = {
  ...
  'x-app-id',        // 🔥 agregar
  'x-db',
  'x-db-key',        // 🔥 agregar (este es el error principal)
  'x-wa-phone-number-id',
  'x-phone-number-id',
  'x-wa-phone',
  'x-wa-broadcast-secret', // 🔥 agregar
  'x-wa-secret',           // 🔥 recomendable
  'accept',
  'origin',
  'referer',
  'cache-control',
...
app.options('*', cors(corsOptions));// Acepta JSON grandes (p.ej. logo en base64 del fallback)
