...
import { getSucursalActual, setSucursal, debugSucursalConfig, testSucursalAPI } from '../lib/api';
import { buildApiUrl, getSucursalActual } from '../lib/api';
...
const SucursalSelector = ({ onSucursalChange, showDebug = false }) => {
  ...
  const verificarEstadisticas = async () => {
    ...
    try {
      ...
      setConnectionStatus(testResult ? 'connected' : 'disconnected');
      
      // Intentar obtener estadísticas del debug endpoint
      const data = await api(`/debug/sucursales?sucursal=${encodeURIComponent(sucursal)}`);
      if (response.ok) {
      ...
      setEstadisticas({
        ...
        estadisticas: {
          doctors: [
            ...
            { sucursal_id: 'sucursal_2', count: '2' }
          ],
          appointments: [
            { sucursal_id: 'sucursal_1', count: '15' },
            { sucursal_id: 'sucursal_2', count: '8' }
          ]
        }
      });
    } finally {
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      window.open(buildApiUrl(`/debug/sucursales?sucursal=${encodeURIComponent(s)}`), '_blank');
                    }}
