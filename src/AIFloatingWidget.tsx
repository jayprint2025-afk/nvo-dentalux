...
export default function AIFloatingWidget(props: { apiBase?: string; sucursalId?: string; dbKey?: string; appId?: string }) {
  ...
  const [wakeWordEnabled, setWakeWordEnabled] = React.useState(() => {
    return localStorage.getItem("f1_wake_word_enabled") === "1";
  });
  const [wakeWordListening, setWakeWordListening] = React.useState(false);
  const [wakeWordError, setWakeWordError] = React.useState<string | null>(null);
...
const buildLeadReport = React.useCallback(() => {
  ...
  const stopWakeWord = React.useCallback(() => {
  ...
  const startWakeWord = React.useCallback(async () => {
    if (!wakeEnabledRef.current) return;
    if (voiceStateRef.current.connected || voiceStateRef.current.connecting) return;
    ...
    if (!SpeechRecognitionCtor) {
      setWakeWordError(
        'Este navegador no permite activación por palabra clave. Usa Edge o Chrome actualizado.'
      );
      setWakeWordListening(false);
      return;
    }
    ...
    try {
      ...
      recognition.onresult = (event: any) => {
        for (let index = event.resultIndex; index < event.results.length; index += 1) {
          ...
          for (let alt = 0; alt < result.length; alt += 1) {
            const transcript = String(result[alt]?.transcript || '')
              .normalize('NFD')
              .replace(/[\u0300-\u036f]/g, '')
              .toLowerCase()
              .replace(/[.,;:!?¿¡]/g, ' ')
            ...
            wakeActivatingRef.current = true;
            setWakeWordListening(false);
            setOpen(true);
            setTab('f1');
            ...
            window.setTimeout(() => {
              ...
              wakeActivatingRef.current = false;
            }, 350);
      ...
      recognition.onerror = (event: any) => {
        ...
        if (code === 'not-allowed' || code === 'service-not-allowed') {
          setWakeWordError(
            'Autoriza el micrófono para activar F1 con la voz.'
          );
          setWakeWordEnabled(false);
          localStorage.setItem('f1_wake_word_enabled', '0');
          wakeEnabledRef.current = false;
        } else if (code !== 'aborted' && code !== 'no-speech') {
          setWakeWordError(`Activación por voz: ${code || 'error del micrófono'}.`);
        }
      };
      ...
      wakeRecognitionRef.current = null;
      setWakeWordListening(false);
      setWakeWordError(error?.message || 'No fue posible activar “Oye F1”.');
    }
  }, [connectVoice]);

  const toggleWakeWord = React.useCallback(async () => {
    const next = !wakeEnabledRef.current;
    wakeEnabledRef.current = next;
    setWakeWordEnabled(next);
    localStorage.setItem('f1_wake_word_enabled', next ? '1' : '0');
    ...
    // Esta llamada ocurre desde un clic del usuario y permite solicitar
    // el permiso inicial del micrófono.
    try {
      ...
      setWakeWordEnabled(false);
      localStorage.setItem('f1_wake_word_enabled', '0');
      setWakeWordError('No se autorizó el micrófono para “Oye F1”.');
    }
              ...
              <div className="h-full flex flex-col bg-gray-50">
                <div className="p-3 overflow-auto flex-1">
                  ...
                  <div className="bg-white border rounded-xl p-3 mb-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-sm font-semibold">Activación por voz “Oye F1”</div>
                        <div className="text-[11px] text-gray-500">
                          Actívalo una vez y di “Oye F1” o “F1” sin presionar el robot.
                        </div>
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-white ${
                          wakeWordEnabled ? 'bg-emerald-600' : 'bg-gray-600'
                        }`}
                      >
                        {wakeWordEnabled
                          ? <Mic className="w-4 h-4" />
                          : <MicOff className="w-4 h-4" />}
                        {wakeWordEnabled ? 'Oye F1 activo' : 'Activar Oye F1'}
                      </button>
                    </div>
                    {!!wakeWordError && (
