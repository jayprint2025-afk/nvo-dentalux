...
export class JarvisVoiceController{
 ...
 stop(){
  this.ttsSeq++; this.fishSpeaking=false; this.stopFishAudio(); this.dc?.close();this.pc?.close();this.stream?.getTracks().forEach(t=>t.stop());
 ...
 private async handleEvent(raw:string){
  ...
  if(e.type==='input_audio_buffer.speech_started'){
   // Mientras Fish habla, speech_started puede ser eco del propio altavoz.
   // Esperamos una transcripción real antes de cancelar la respuesta.
  ...
  if((e.type||'').includes('input_audio_transcription')&&userText){
