...
export default function AIFloatingWidget(props: { apiBase?: string; sucursalId?: string; dbKey?: string; appId?: string }) {
  ...
  const loadLeads = React.useCallback(async () => {
    try {
      ...
      // auto select
      if (!selectedLead && list.length) {
        setSelectedLead(list[0]);
      } else if (selectedLead) {
        // refresca referencia
        const updated = list.find((x: any) => x.id === selectedLead.id);
  ...
  return (
    <>
      ...
      open && (
        ...
        >
          <div
            className="fixed bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden"
            style={{
              right: 12,
              bottom: 108, // arriba del bot
          ...
          {/* Header draggable */}
          <div
            className="flex items-center justify-between gap-2 px-3 py-2 bg-gray-50 border-b  select-none"
